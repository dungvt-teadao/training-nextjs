    ...
    return (
        <nav className="bg-gray-800 flex justify-between items-center h-20 p-4 ">
            ...
            <ul className="flex gap-6 list-none text-gray-200 font-semibold">
                {links.map((link) => (
                    <li key={link}>
                        <Link href={link.toLowerCase()} className="p=2 hover:bg-gray-700 hover:rounded-md active:bg-purple-700 focus:ring focus:rounded-md focus:outline-none focus:ring-green-600"
                        >
                           {link}
                        </Link>
                    </li>
